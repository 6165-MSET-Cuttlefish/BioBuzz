...
`modules/LimelightCamera` is the AprilTag half of vision and shares nothing with the webcam `Camera` above: it owns the Limelight 3A (config name `limelight`) and answers one question — is the HIVE cell scorable, or has it tipped down? **The verdict is orientation**, per the manual: a scorable cell shows its AprilTag cluster right-side up and an unscorable one shows it upside-down, so the test is `|roll| < 90`. In frame and right-side up is scorable; upside-down for `HOLD_SECONDS` is tipped, and so is nothing in frame at all for that long, since a cell can also turn its tags away entirely. An upright sighting clears a tip at once.
