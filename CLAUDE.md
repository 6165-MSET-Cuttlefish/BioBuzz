...
The 2026–2027 competition-season repository for **FIRST Tech Challenge team 6165 MSET Cuttlefish**, for the **BIOBUZZ™** game (kickoff September 12, 2026). The game element is **Pollen**, ~2.8" yellow plastic balls; `modules/vision/` detects and tracks them.
...
**Nothing in this repo has run on a robot yet.** The SDK 12.0.0 build, Sloth, slothboard, Pedro 3.0.0, and Ivy were verified by compiling, jar diffs, and review only. Claude keeps the on-robot verification checklist in its own memory; ask for it before the first hub session and work it in order.
...
**AutoTune:** deploy, join the robot wifi, open `http://192.168.43.1:10158` (websocket on 12649; over USB forward both and use `localhost`). Run **Mecanum**, **Pinpoint** (it measures directions and offsets itself), **Foresight**, then **Tests**, and paste each emitted block over the matching block in `Constants.java`, re-adding `c.brakeAtEnd.set(true)` after the Foresight paste. The Tests procedure ignores its Distance field and always drives 48 in (upstream bug), so leave it at 48 and clear a 48 × 48 in area. Until Foresight has been tuned, only follow paths with the wheels off the ground. The Pinpoint strafe direction (`FORWARD`) is a guess; run the Pinpoint procedure before trusting the pose.
...
- **Camera Module Test** runs `modules/Camera` through the framework. **BioBuzz Tele** is the robot-centric drive smoke test.
