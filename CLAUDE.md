...
`stop()`: `Scheduler.reset()`, which does not run command `end()` hooks; then `follower.stop()` and a zero write to Pedro's drive motors, `module.stop()` on every module, and `onEnd()`, each guaranteed to run even if an earlier one throws, with the first exception rethrown afterwards. The SDK fail-safes every REV hub before the OpMode's `stop()`; a module's `stop()` covers what that misses, like the webcam and Limelight.
...
7. **Ball detection on the Limelight**: nothing is live-tunable. Every constant is at the top of `limelight/ball_detection_snapscript.py`, mirroring `BallVisionConstants`' names; edit it in the Limelight web editor (which reloads the pipeline) with `DISPLAY_MODE = "MASK"` to see the gates, then copy what worked back into this file.
...
**Hub configs.** Every framework OpMode needs `fl`, `bl`, `fr`, `br` and `pinpoint`, because `Robot` always builds the follower. Every BioBuzz and Decode OpMode also needs `Drivetrain`'s `floodgate` (an Analog Input) and `leftPto`/`rightPto` (Servos, held at `ptoConfig` UP on every write so the drive stays decoupled from the lift), and `limelight` (`LimelightCamera`, or the Turret on Decode). The camera OpModes need `nerdDetector` (the webcam; the name lives only in `Camera.WEBCAM_NAME`). `res/xml/` ships `cuttle_decode.xml` (the Cuttle bot, for every framework OpMode on it including the DECODE ones) and `camera.xml` (webcam only, for **Camera Tune** and **Ball Vision**; breaks every framework OpMode).
...
- **Generated code** (`limelight/pipelines/*`) is never hand-edited; regenerate it with `limelight/generate_pipelines.py` after editing `limelight/cell_tip_snapscript.py` or `LimelightCamera`'s tag ids. `limelight/ball_detection_snapscript.py` is not a template and not generated: edit and upload it directly.
