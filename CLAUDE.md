...
`modules/LimelightCamera` is the AprilTag half of vision and shares nothing with the webcam `Camera` above: it owns the Limelight 3A (config name `limelight`) and answers one question — is the HIVE cell in view scorable, or has it tipped down? A cell's AprilTag cluster reads right-side up while the cell accepts scoring elements and upside-down once it has tipped, so the test is the manual's: a cluster is upright when `|roll| < 90`.
...
**Roll comes from the tag quads, not a pose solve.** The SDK's `AprilTagDetection.ftcPose.roll` is a 6-DOF result; a SnapScript has no intrinsics and no tag size, so there is no pose to solve. But roll is rotation about the camera's viewing axis, which is exactly a tag's apparent rotation in the image, and aruco's corner order gives that directly — `c0->c1` is the tag's top edge, pointing right (roll ~0) when upright and left (roll ~180) when upside-down. The number is not the SDK's field but measures the same angle, and it reproduces the manual's worked examples (-19.7° and 159.3°) to about half a degree. A cluster's roll is the *circular* mean over its visible tags, so one badly-decoded corner cannot flip the verdict and angles either side of ±180 average correctly.

Which cluster the verdict is about is whichever of the alliance's two has more tags in frame, with an upright one winning a tie — the robot can act on an upright cell. `isTipped()` requires the targeted cluster to have read upside-down continuously for `HOLD_SECONDS`; an upright sighting clears it at once, and losing sight of the cluster does not advance the hold, so an empty frame never reports tipped. `isScorable()` is the same verdict from the other side, undebounced. `RobotActions.checkTip()` is the auto-facing form, an Ivy command that finishes when the cell tips, with its own timeout (`checkTipTimeoutMs`, 10 s; there is a `checkTip(timeoutMs)` overload) so it cannot stall an auto — which means finishing does not mean tipped, and a caller that cares must read `isTipped()` afterwards. The script echoes its alliance's id checksum and the module discards any verdict that doesn't match, so a pipeline index pointing at the wrong script surfaces as a permanent "NO VERDICT" rather than a wrong answer. `llpython` never carries NaN — a roll with no cluster in view is sent as `ROLL_NONE` (999) and mapped back to NaN in Java — because it ships inside the Limelight's results JSON, where a bare `NaN` token can cost the hub the whole payload.

Each pipeline also draws its whole state onto the frame the Limelight's own web UI streams, which is the only telemetry a SnapScript has: every tag outlined and labelled, green when its cluster is upright, red when upside-down and grey for the other alliance's; a line per cluster with its id range, visible count, roll and UP/DOWN, marking which one is the target; a **SCORABLE** / **TIPPED** / no-cluster banner; and the hold timer against its threshold. `DRAW_OVERLAY` turns the lot off. It is how you tell a cell that is genuinely tipped from a camera that cannot resolve the tags at all.
