...
- **Cell Tip Test** (`opmodes/test/CellTipTest`, on `cuttledecode.xml`) shows the live cell verdict and runs `RobotActions.checkTip`; run it after uploading scripts. A cell of your alliance right-side up should read SCORABLE, and upside-down or out of frame should read TIPPED a quarter second later. `Cell Tip Test → alliance` applies on the next init.
