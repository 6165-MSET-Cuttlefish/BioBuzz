...
- **The Driver Station must be on 12.0**: an 11.x DS runs OpModes but fails the inspection screen. Install `FtcDriverStation-release.apk` from the `FtcRobotController` `v12.0` release.
- **JDK 17** must be installed; the Gradle daemon is pinned to it in `gradle/gradle-daemon-jvm.properties`. Gradle 9.1.0 and AGP 8.13.2 need Android Studio Narwhal 3 Feature Drop or newer.
...
Vision (`modules/vision/`, on the Cuttle bot) detects Pollen and red/blue Nectar. `TrackedBall` is camera-relative, so a stationary ball reads as moving when the robot moves; `FieldBall` is field-relative (from the robot pose at the frame's capture time) and exists only after `Camera.withFollower(...)`. `FieldBallTracker` keeps a ball's ID when the robot turns away and back; an out-of-view ball stays in `getFieldBalls()` and the nearest/fastest/moving getters at its last position, with `visible()` false, for `forgetAfterSeconds` (10 s), so check `visible()` before chasing one. `BallVisionConstants` holds every detection number: the thresholds are live `@Config` (`BallVision_*`), while `H_ARRAY` and the ROI/Hough constants are `static final`; `BallVisionDisplay` picks the camera view. `eocvsim/balldetection/` is an older standalone copy of the detector for EOCV-Sim, which can't import TeamCode; `eocvsim/homography/` produces `H_ARRAY` from the calibration chessboard, printed with a white border about one square wide. `H_ARRAY` is in 640x480 pixels, the size `WebcamSession` streams, so the EOCV-Sim source must be 640x480 too.

`modules/LimelightCamera` owns the Limelight 3A and reports whether this alliance's HIVE cell has tipped (`isTipped()`) or is scorable (`isScorable()`), shown as a `Cell` telemetry line. `RobotActions.checkTip()` is the Ivy form; it also finishes after `checkTipTimeoutMs`, so read `isTipped()` afterwards. A SnapScript on the Limelight decides: this alliance's cluster upside-down is scorable, right-side up or out of frame is tipped, each after holding 0.25 s. That inverts the roll heuristic in FIRST's "AprilTag Clusters" Tech Tip, so mount the Limelight image-upright, looking the way the launcher launches. Pipelines 1 (red) and 2 (blue) hold the scripts and are chosen at module init from `Context.allianceColor`, so set the alliance before INIT, or in `createRobot()` (`initialize()` is too late). Pipeline 0 is DECODE's AprilTag pipeline, owned by `decode/modules/Turret`, so `DecodeRobot` never builds `LimelightCamera`; the Limelight keeps its last pipeline, so re-INIT a Decode OpMode run after Cell Tip Test (the Turret shows `WRONG PIPELINE`). The `llpython` slots are listed in `LimelightCamera`'s `OUT_*` constants and the script's docstring; edit them together.

To change the script or the tag ids, edit it (the generator reads the ids and pipeline indices from `LimelightCamera`), run `limelight/generate_pipelines.py`, and upload each file in `limelight/pipelines/` at `http://limelight.local:5801` (pipeline index from the file name; Input tab: type Python, exposure 1000, which the hub cannot set). A wrong script or pipeline shows NO VERDICT, never a wrong verdict.

`limelight/ball_detection_snapscript.py` is `BallDetectionPipeline`'s detector as a SnapScript, for running ball detection on the Limelight instead of the webcam. Same algorithm and same HSV numbers, but no tracking: it reports each frame's detections with no ids and no velocities, so the hub would feed them into `BallTracker` itself. It is not generated and has no per-alliance variants, so edit this file and upload it as-is to pipeline 3 (0-2 are taken). Its `llpython` is `SCRIPT_ID`, a ball count, then three doubles per ball (type code, x, y in camera-frame ground inches), nearest first because 32 doubles only hold 10 balls; the docstring has the slot table. Its `H_ARRAY` starts as the webcam's and means nothing until it is recalibrated for the Limelight's lens with `eocvsim/homography`; `CALIBRATION_SIZE` is the resolution that calibration was done at, and contact points are scaled to it, so the Limelight's streaming resolution need not match. One pipeline runs at a time, so ball detection and the cell-tip verdict cannot both be live. No hub module reads it yet.
...
6. **Cell-tip detection**: `LimelightCamera`'s `@Config` fields on the hub; everything else is in `limelight/cell_tip_snapscript.py`.
7. **Ball detection on the Limelight**: nothing is live-tunable. Its constants are at the top of `limelight/ball_detection_snapscript.py`; edit them in the Limelight web editor with `DISPLAY_MODE = "MASK"` to see the gates, then copy what worked back into this file.
...
**Hub configs.** Every framework OpMode needs `fl`, `bl`, `fr`, `br` and `pinpoint`; BioBuzz and Decode OpModes also need `floodgate` (an Analog Input), `leftPto`/`rightPto` (Servos) and `limelight`; the camera OpModes need `nerdDetector` (the name lives only in `Camera.WEBCAM_NAME`). `res/xml/` ships `cuttle_decode.xml` (the Cuttle bot, for every framework OpMode) and `camera.xml` (webcam only, for **Camera Tune** and **Ball Vision**).
...
- **Cell Tip Test** (`opmodes/test/CellTipTest`, on `cuttle_decode.xml`) runs `RobotActions.checkTip` against the live verdict after uploading scripts: your alliance's cluster upside-down should read SCORABLE, right-side up or out of frame TIPPED.
...
- **Distance sensors** (`opmodes/test/distance/`): bench tests, address setup and the predictive-braking experiment for the laser, ultrasonic and REV sensors; they read the hub config names `Laser`, `sonar` and `distance`.
...
- **Generated code** (`limelight/pipelines/*`) is never hand-edited; regenerate it. `limelight/ball_detection_snapscript.py` is not generated: edit and upload it directly.
