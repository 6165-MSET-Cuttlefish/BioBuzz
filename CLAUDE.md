...
3.0.1 fixed `Curve.pathCompletion`, which 3.0.0 had inverted on straight lines and multi-leg paths, so `Interpolator.linear`, `Path.linear(...)`, and `Follower.completion()` are usable again; `HeadingInterpolatorBuilder` stays for multi-phase heading profiles. 3.0.1 also sleeps 500 ms in the Pinpoint localizer's constructor for the IMU reset, so every framework OpMode's INIT takes half a second longer.
