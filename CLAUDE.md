...
`modules/LimelightCamera` is the AprilTag half of vision and shares nothing with the webcam `Camera` above: it owns the Limelight 3A (config name `limelight`) and answers one question — is the HIVE cell scorable, or has it tipped down? **The verdict is visibility.** A cell that is up shows its AprilTag cluster to the camera; a cell that has tipped down points its cluster away, so it leaves the frame entirely rather than staying readable upside-down. `MIN_VISIBLE_TAGS` of the alliance's ids in frame means scorable; none for `HOLD_SECONDS` means tipped, and one tag returning clears it at once.

`Cluster` names the two cells by the side of the field they face: RED's scoring-side cell is 30-33 and its audience-side one 34-37, while BLUE's are the other way round — 42-45 scoring, 38-41 audience. The id runs are therefore *not* in the same order for both alliances, so the cluster code in `llpython[4]` is the side (0 = scoring, 1 = audience), never the position, and the scripts always list scoring first. `LimelightCamera`'s four `*_IDS` arrays are the one place those runs are written down; the checksums and both scripts derive from them. Which cell is in view also tells you which half of the field the robot is on.

There is one Limelight pipeline per alliance, holding its own copy of the SnapScript with that alliance's eight ids — both of its cells — and the roll test baked in, so the hub writes nothing over the link at any point: it calls `pipelineSwitch` once in `init()` for `Context.allianceColor` (`LimelightCamera.redPipeline` 1, `bluePipeline` 2) and then only reads `llpython` (tipped, scorable, roll, visible count, which cluster, the other cluster's visible count, an alliance checksum, a frame counter; the slot layout is declared as constants at the top of `LimelightCamera` and in the scripts' docstrings, and they have to be edited together). The manual's first targeting condition — only target your own alliance's clusters — is structural rather than a check, since a pipeline only knows its own alliance's ids. Because the pipeline is chosen at init, `Context.allianceColor` has to be set before the OpMode initializes (in `createRobot()`); changing it later does nothing. No tag ids live in Java beyond the two checksums the freshness guard needs, and none live in `architecture/`.

**Roll is measured and reported, but does not gate the verdict by default.** The manual's `|roll| < 90` test assumes a tipped cluster stays in view reading upside-down; in practice it usually just leaves the frame, which is why visibility decides and `REQUIRE_UPRIGHT` (off) is what adds the roll test on top. Roll itself does not come from a pose solve: the SDK's `AprilTagDetection.ftcPose.roll` is a 6-DOF result, and a SnapScript has no intrinsics and no tag size. But roll is rotation about the camera's viewing axis, which is exactly a tag's apparent rotation in the image, and aruco's corner order gives that directly — `c0->c1` is the tag's top edge, pointing right (roll ~0) when upright and left (roll ~180) when upside-down. The number is not the SDK's field but measures the same angle, and it reproduces the manual's worked examples (-19.7° and 159.3°) to about half a degree. A cluster's roll is the *circular* mean over its visible tags, so one badly-decoded corner cannot skew it and angles either side of ±180 average correctly.

Which cell the verdict is about is whichever of the alliance's two has more tags in frame, with an upright one winning a tie. One timer covers both criteria: the cell is scorable when enough of its cluster is in frame — and upright too, if `REQUIRE_UPRIGHT` is on — and `isTipped()` fires once it has not been scorable for `HOLD_SECONDS` straight. `isScorable()` is the undebounced other side. With `REQUIRE_SEEN` off (the default) an empty frame reports tipped once the hold elapses, so a camera pointed away from the hive reads the same as a tipped cell; turning it on withholds that until the cluster has been seen at least once. `RobotActions.checkTip()` is the auto-facing form, an Ivy command that finishes when the cell tips, with its own timeout (`checkTipTimeoutMs`, 10 s; there is a `checkTip(timeoutMs)` overload) so it cannot stall an auto — which means finishing does not mean tipped, and a caller that cares must read `isTipped()` afterwards. The script echoes its alliance's id checksum and the module discards any verdict that doesn't match, so a pipeline index pointing at the wrong script surfaces as a permanent "NO VERDICT" rather than a wrong answer. `llpython` never carries NaN — a roll with no cluster in view is sent as `ROLL_NONE` (999) and mapped back to NaN in Java — because it ships inside the Limelight's results JSON, where a bare `NaN` token can cost the hub the whole payload.

Each pipeline also draws its whole state onto the frame the Limelight's own web UI streams, which is the only telemetry a SnapScript has: every tag outlined and labelled, green for this alliance's (red only when the roll gate is on and rejects it) and grey for the other alliance's; a line per cell — SCORING or AUDIENCE, its id range, visible count, roll and UP/DOWN — marking which one is the target; a **SCORABLE** / **TIPPED** banner; and the not-scorable timer against its threshold, with which criteria are in force. `DRAW_OVERLAY` turns the lot off. It is how you tell a cell that is genuinely tipped from a camera that cannot resolve the tags at all.

The two uploaded scripts are generated, not hand-kept: `limelight/cell_tip_snapscript.py` is the template and `scripts/generate-limelight-pipelines.py` stamps it out into `limelight/pipelines/pipeline{n}_{alliance}.py`, filling in `ALLIANCE`, `PIPELINE_INDEX` and `CLUSTERS` from the pipeline indices and the four `*_IDS` arrays it reads straight out of `LimelightCamera.java` so the two cannot drift. Edit the template, re-run the generator, re-upload both; never edit a file under `limelight/pipelines/`.
...
6. **Cell-tip detection** is tuned in two places, and only one of them is live. On the hub, `LimelightCamera`'s `redPipeline`, `bluePipeline`, `maxStalenessMs` and `checkTipTimeoutMs` are ordinary dashboard `@Config` fields. Everything the detection itself does — `CLUSTERS`, `HOLD_SECONDS`, `MIN_VISIBLE_TAGS`, `MIN_TAG_AREA_PX`, `REQUIRE_UPRIGHT`, `UPRIGHT_MAX_ROLL_DEG`, `REQUIRE_SEEN`, `DRAW_OVERLAY` — lives in `limelight/cell_tip_snapscript.py` and only changes by editing the template, re-running `scripts/generate-limelight-pipelines.py` and re-uploading; there is deliberately no way to push any of it at run time. Upload from the Limelight web UI at `http://limelight.local:5801`: select pipeline index *n*, Input tab, pipeline type Python, paste `limelight/pipelines/pipeline{n}_*.py`, save — both, each at the index its filename names, matching `LimelightCamera`'s pipeline fields.
