...
`modules/LimelightCamera` is the AprilTag half of vision and shares nothing with the webcam `Camera` above: it owns the Limelight 3A (config name `limelight`) and answers one question — has the watched cell tipped over onto its four tags? Every part of that answer is computed on the Limelight by `limelight/cell_tip_snapscript.py`, a SnapScript pipeline: it detects 36h11 tags, matches them against the four ids the hub asked for, and times the hidden window itself, because a 40 Hz OpMode loop cannot time a 0.25 s window against a 90 fps camera without smearing it. The hub side is two array copies per loop — `llrobot` out (the four tag ids, the hold time, enabled, the minimum tag area, require-seen) and `llpython` back (tipped, visible count and mask, hidden seconds, seen-since-arm, tag-set checksum, tags in frame, frame counter) — with the slot layout declared once as constants at the top of `LimelightCamera` and in the script's docstring; they have to be edited together. The checksum is what makes switching cells safe: the module only believes a verdict whose echoed checksum matches the cell it is currently watching, so a leftover answer about the previous cell can never satisfy the next one. `Cell` holds the four tag sets (`RED_1` 30-33, `RED_2` 34-37, `BLUE_1` 38-41, `BLUE_2` 42-45). `RobotActions.checkTip(cell)` is the auto-facing form — an Ivy command that finishes when the cell tips — and `LimelightCamera.checkTip(cell)` the plain boolean; both report false whenever the answer is stale, absent, or about another cell. The command carries its own timeout (`checkTipTimeoutMs`, 10 s; there is a `checkTip(cell, timeoutMs)` overload) so it cannot stall an auto on a cell that never tips or a Limelight that never answers — which means finishing does not mean tipped, and a caller that cares must read `isTipped()` afterwards.
...
6. **`LimelightCamera`** for cell-tip detection: `snapScriptPipeline` (which Limelight pipeline holds the SnapScript), `hiddenHoldSeconds` (0.25 — pushed to the script, which does the timing), `minTagAreaPx` (raise it to ignore tags across the field), `requireSeenBeforeTip` (refuse to report a tip for a cell that was never in view), `maxStalenessMs`, and `checkTipTimeoutMs` (how long `RobotActions.checkTip` waits before giving up). Upload the script from the Limelight web UI at `http://limelight.local:5801` — Input tab, pipeline type Python, paste `limelight/cell_tip_snapscript.py`, save, then point `snapScriptPipeline` at that index. With `llrobot` all zeros the script watches `DEFAULT_IDS` so the web UI's own preview works on the bench.
...
**Hub configuration names**: every framework OpMode needs `fl`, `bl`, `fr`, `br` (motors) and `pinpoint` (goBILDA Pinpoint), because `Robot` builds the follower unconditionally. Optional: `floodgate` (analog current sensor), `nerdDetector` (webcam, only for the ball-vision tests) and `limelight` (Limelight 3A, only for cell-tip detection — `LimelightCamera` `tryGet`s it, so its absence degrades to "no verdict" rather than breaking unrelated OpModes). `res/xml/camera.xml` is a minimal bench config carrying only `nerdDetector` and `limelight` — enough for the bare-`LinearOpMode` tests (**Ball Vision**, **Cell Tip Test**) and nothing else, since `Robot` builds the follower unconditionally; `res/xml/cuttle_decode.xml` is the full robot.
